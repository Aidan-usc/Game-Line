...
export function projectedPayout(stake, americanOddsArray) {
...
// Public entry
export function initParlayPage(sport) {
  state.sport = sport;
...
function toggleLeg({eid, market, selection, line, odds, label, game}) {
  // rule: only one pick per market per event. Selecting again replaces it.
  // also cap to max legs total.
  ...
  if (state.legs.length >= state.maxLegs) {
    alert(`Max ${state.maxLegs} legs reached.`);
    return;
  }
...
function markSelections({eid, market}) {
  // set .selected on the two buttons that match current leg (if any)
  ...
  card.querySelectorAll(".pick-btn").forEach(b => {
    ...
    if (p.market === market && p.selection === leg.selection) {
...
function hookRail() {
  ...
  document.getElementById("submit-parlay").addEventListener("click", () => {
...
function updateRail() {
  ...
  ul.querySelectorAll(".remove").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // clear selected style on the card too
  ...
  document.getElementById("total-odds").textContent = american;
  document.getElementById("payout").textContent = `$${payout.toFixed(2)}`;
  document.getElementById("submit-parlay").disabled = state.legs.length === 0;
}
